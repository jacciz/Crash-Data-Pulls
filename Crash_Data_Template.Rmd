...
years = c("19") # select years to import, must be in this format
...
```{r}
speed_drivers <- persons_new %>% get_speed_flag() %>% filter(speed_flag == "Y") %>% select(CRSHNMBR) %>% distinct()

crashes_new %>%  inner_join(., speed_drivers, by = "CRSHNMBR") %>% 
  select(CRSHNMBR, TOTINJ, TOTFATL, starts_with("RDCOND")) %>%
  filter_all(any_vars(grepl("Snow|Ice|Slush", .))) %>% count(sum(TOTINJ), sum(TOTFATL))
```
