...
Functions include:
  + Gather all OWI cases by performing a text search of all the statute_citations column
  + Gather sentencing data of certain case types (done either by a specific sentenceCode or conditionCode)
  + Merges all the sentencing data into one dataframe
  + I run these functions for each charge in a case. I found most cases have 12 or fewer charges
...
# Not using this, 
# third_off <- get_third_plus_offenders(owi_cases)
# third_off_data <- semi_join(sel_cases, third_off, by = "id")

# third_off_data$charges.jdgmts.timeConds.condLen1 %>% unique()
# cases$charges.jdgmts.timeConds.condLen1 %>% unique()
