...
---
This script gathers court case data from an API. Data comes in JSON format. Output is one long flattened files of court cases given a date range. The API works in two steps - 1) Gather court case number and county code for each day, then 2) Uses case number / county code to get actual court case data. I wrote a final function that combines these two functions. Output is saved in an RDS format for data analysis (see get_third_offenders.RMD).
...
get_allcasedata_complete <-
  function(b = baseurl, code = wcisClsCod, start, end, day_int) {
    ...
    # First, we get long df with all casenumbers/county code in the daterange
...
# save as csv as a backup - RENAME so you don't overwrite file
